...
from Module import tushare_api, MySQL_Database, mysql_data_storage as mds
from TOKEN_ID import TOKEN
from config import DATABASE, STOCK_BASIC_NAME, STOCK_BASIC_
...
class TuShareStock:
    def __init__(self):
        ...
        self.STOCK_BASIC_NAME = STOCK_BASIC_NAME
        self.DELISTED_STOCK_NAME = self.STOCK_BASIC_NAME + '_' + self.stock_basic_API.lower()
        ...
        if self.stock_basic_API == 'L':
            self.stock_name = STOCK_BASIC_NAME
        ...
        if self.stock_basic_API == 'L':
            ...
            self.stocks_list = tushare_api.stock_basic()  # list_status默认='L'
            self.ts_code = self.stocks_list.ts_code

            # 调用完stock_basic接口后直接存储进'all_stock_basic'数据表中，不再使用`stock_basic_save_into_db()`函数进行存储
            DB.write_data(self.stocks_list, self.STOCK_BASIC_NAME, if_exists='replace')
            ...
            self.delisted_stock_list = tushare_api.stock_basic(list_status=self.stock_basic_API)

            # 将从tushare网站获取的已经退市的stock_basic写入MySQL数据库的'all_stock_basic_D'表中
            # 因为暂时用不上退市的股票，所以如果获取退市的股票，则将该数据直接存储进数据库中
            DB.write_data(self.delisted_stock_list, self.DELISTED_STOCK_NAME, if_exists='replace')
        else:
...
# # ################ stock_basic数据表相操作 # ################
# 不需要比对'stock_basic'数据表中的具体数据和长度，有改变直接替换即可
...
def main():
    ...
    if is_trade:
        ...
        if mysql_stock_list.stock_basic_API == 'L':
            # 获取TS网站的stock_basic数据并存储到数据表相关操作
            # tscode = mysql_stock_list.get_ts_code()  # 确认调用了stock_basic接口，获取tushare上stock_basic的ts_code
            tscode = mysql_stock_list.ts_code  # 获取tushare上stock_basic的ts_code
        ...
        # 各个股票的数据表相关操作
        mds.get_daily_stocks(DB, tscode, tables)
