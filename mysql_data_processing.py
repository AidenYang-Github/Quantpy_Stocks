...
def read_bak_basic_data(tb_name, ts_code):
    """
    根据表名，读取表中的数据
    :param tb_name: 表名
    :param ts_code: 股票代码
...
def main():
    ...
    for tb_name in tables:
        if tb_name[:2] == 'bj' or tb_name[:2] == 'sh' or tb_name[:2] == 'sz':
            # 读取到股票
            df = read_data(tb_name)
            read_detail_data(df)


def main_single_stock(stock_name):
    """
    ...
    # sn = stock_name.split('.')
    data = read_data(tscode_to_tbname(ts_code=stock_name))
    bak_basic_data = read_bak_basic_data(tb_name='all_bak_basic_20230318', ts_code=stock_name)
...
if __name__ == '__main__':
    ...
    if func_num == 0:
        ...
        main_single_stock(ts_code)  # 开放ts_code
        print('Done!')
    elif func_num == 2:
